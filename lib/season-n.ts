...
function getRawRound(
  data: RawSeasonNData,
  round: number,
  answerConfigMap?: AnswerConfigMap
): RawRound {
  return data.rounds[String(round)] ?? {
    questionCount: getSeasonQuestionCount("N", round, answerConfigMap),
    rows: [],
  };
}
...
function buildRoundRows(
  ...
  for (const row of deduped) {
    ...
    const answers = row.answers.map((choice) => normalizeChoice(choice));
    const scoreFromAnswers = scoreAnswersForSeason("N", round, answers, answerConfigMap);

    rows.push({
      studentId: matched.id,
      className: normalizeDisplayClassName(matched.class_name || row.className || "미분류"),
      score: scoreFromAnswers ?? normalizeScore(row.score),
      answers,
    });
  }
...
export function buildSeasonNViewData(
  ...
  for (let round = 1; round <= ROUND_COUNT; round += 1) {
    const rawRound = getRawRound(data, round, answerConfigMap);
    ...
    const rows = buildRoundRows(round, mergedRound, students, answerConfigMap);
    const scores = rows
      .map((row) => row.score)
    ...
    const answerKey = getSeasonAnswerKey("N", round, answerConfigMap) ?? [];
    const questionCount = Math.max(
      mergedRound.questionCount,
      getSeasonQuestionCount("N", round, answerConfigMap),
      answerKey.length
    );
...
export function buildSeasonNAdminStats(
  ...
  const rawRound = getRawRound(data, safeRound, answerConfigMap);
  const mergedRound: RawRound = {
    questionCount: rawRound.questionCount,
    rows: mergeRoundRows(rawRound.rows, uploadedRowsByRound[safeRound] ?? []),
  };
  const rows = buildRoundRows(safeRound, mergedRound, students, answerConfigMap);
  const scores = rows
    .map((row) => row.score)
    .filter((score): score is number => score !== null);
  const answerKey = getSeasonAnswerKey("N", safeRound, answerConfigMap) ?? [];
  const questionCount = Math.max(
    mergedRound.questionCount,
    getSeasonQuestionCount("N", safeRound, answerConfigMap),
    answerKey.length
  );
